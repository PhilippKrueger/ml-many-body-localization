...
def get_wc(N, n, Ws, repetitions):
    ...
    state_prediction_w_averaged = []
    for W in Ws:
        item = [np.mean([item[0] for item in state_prediction_w if item[1] == W]), W]
        if not np.isnan(item[0]):
            state_prediction_w_averaged.append(item)
    print(state_prediction_w_averaged)
    state_prediction_w_averaged = np.asarray(state_prediction_w_averaged)
    print(state_prediction_w_averaged)
    popt, pcov = curve_fit(logistic, state_prediction_w_averaged[:,0], state_prediction_w_averaged[:,1])  # state_prediction.astype(np.float))
    # plot_wc_fit(N,popt,state_prediction)
    return popt[0], state_prediction_w_averaged[:,0], state_prediction_w_averaged[:,1] # Wc, Ws, predicitons
...
class HeatMapPlotter:
    ...
    def plot_wc_heatmap_n(self):
        ...
        for N in self.Ns:
            ...
            fig, ax = plt.subplots()
            plt.title(
                "Predicted phases and critical disorder strength $W_c$ \n over block size $n$ at system size $N=$" + str(
                    N))
    ...
    def plot_wc_heatmap_N(self):
        ...
        for n in range(1, self.n_max + 1):
            ...
            fig, ax = plt.subplots()
            plt.title(
                "Predicted phases and critical disorder strength $W_c$ \n over system size $N$ with block size $n=$" + str(
                    n))
